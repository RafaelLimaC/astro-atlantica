...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, className, labelFor, type, ...props }, ref) => (
    <div className={cn("relative", className)}>
      <input
        type={type}
        id="floating_outlined"
        className={cn(
          "peer block w-full appearance-none rounded-3xl border-2 border-[#d4d4d2] bg-transparent p-4 outline-none placeholder:text-[#8b8b8b] focus:ring-0",
        )}
        placeholder=" "
        {...props}
        ref={ref}
